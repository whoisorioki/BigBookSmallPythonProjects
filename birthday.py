    ...
    for a, birthdayA in enumerate(birthdays):
        for b, birthdayB in enumerate(birthdays[a + 1 :]):
            if birthdayA == birthdayB:
                return birthdayA # Return thr matching birthday.

# Display the intro:
print('''Birthday Paradox, by Adrian Orioki
...
This program does a Monte Carlo simulation (that is, a repeated random simulations) to explore this concept.

(It's not actually a paradox, it's just a suprising result.)
''')

# Set up a tuple of months in order:
MONTHS = ('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec')

while True: # Keep asking untill the user enters a valid amount.
